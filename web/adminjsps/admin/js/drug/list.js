...
function batchDelete() {
    ...
    $(":checkbox[name=checkboxBtn]:checked").each(function () {
        drugIdArray.push($(this).val());//把复选框的值添加到数组中
    });
    if (confirm("您是否真要删除所选中的商品？")) {
        location = "/shop_drug_war_exploded/AdminDrugServlet?method=batchDelete&drugIds=" + drugIdArray;
    }
}
...
function batchSoldout() {
    ...
    if (confirm("您是否真要删除所选中的商品？")) {
        location = "/shop_drug_war_exploded/AdminDrugServlet?method=batchSoldout&drugIds=" + drugIdArray;
    }
}
...
function batchPutaway() {
    ...
    if (confirm("您是否真要删除所选中的商品？")) {
        location = "/shop_drug_war_exploded/AdminDrugServlet?method=batchPutaway&drugIds=" + drugIdArray;
    }
}