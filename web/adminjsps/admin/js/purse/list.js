...
function batchDelete() {
    ...
    var purseIdArray = new Array();
    $(":checkbox[name=checkboxBtn]:checked").each(function () {
        purseIdArray.push($(this).val());//把复选框的值添加到数组中
    });
    if (confirm("您是否真要删除所选中的用户？")) {
        location = "/shop_drug_war_exploded/PurseServlet?method=batchDelete&purseIds=" + purseIdArray;
    }
}
...
function batchCheck() {
    ...
    if (confirm("您是否真要对所选中的申请进行处理？")) {
        location = "/shop_drug_war_exploded/PurseServlet?method=batchCheck&purseIds=" + purseIdArray;
    }
}