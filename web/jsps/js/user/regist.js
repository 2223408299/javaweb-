...
function validateReloginpass() {
	...
	if(!value) {// 非空校验
		...
		$("#reloginpassError").css("display", "");
		$("#reloginpassError").text("密码长度必须在3 ~ 20之间！");
		bool = false;
...
function validateVerifyCode() {
	...
	if(!value) {//非空校验
		...
		$.ajax({
			...
			url: "/shop_drug_war_exploded/UserServlet",
...
function _hyz() {
     ...
     * 3. 使用毫秒来添加参数
     */
	$("#imgVerifyCode").attr("src", "/shop_drug_war_exploded/VerifyCodeServlet?a=" + new Date().getTime());
}
